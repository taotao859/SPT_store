      ...
      <el-header>
        <el-row>
          ...
          <el-col :span="20" style="text-align: right">
            ...
            <el-button icon="el-icon-s-home" circle style="border: none;background: none"
                       @click="$router.push({path: '/bossHomePage'})"></el-button>
            <el-button id="log-out-button" icon="el-icon-switch-button"
                       @click="$router.push({path: '/login'})"></el-button>
          </el-col>
        </el-row>
      </el-header>
      <el-main>
        <el-row :gutter="20">
          <el-col :span="4">
            <el-input v-model="customerName" placeholder="客户姓名" required="true"></el-input>
          </el-col>
          <el-col :span="4">
          ...
          <el-col :span="4">
            <el-input v-model="customerContact" placeholder="客户联系方式" type="text" minlength="11" maxlength="11"
        ...
        <br><br>
        <el-table :data="customerList" style="width: 100%" height="600px"
                  :header-cell-style="{background:'#eef1f6',color:'#606266'}" @cell-click="handle">
...
export default {
  ...
  methods: {
    ...
    // 客户种类到01的对应还没确定好
    addItem () {
      this.customerTag = this.customerTag !== '零售客户'
      this.$axios.get('/customer/add?name=' + this.customerName + '&tag=' + this.customerTag + '' + '&contact=' + this.customerContact)
        .then(Response => {
